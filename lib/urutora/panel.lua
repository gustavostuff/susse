...
function panel:constructor()
  ...
  self._maxx = (self.cols) * (self.cellWidth or 0)
...
function panel:calculateRect(row, col)
  local w, h = self.cellWidth or (self.w / self.cols), self.cellHeight or (self.h / self.rows)
...
local function _drawScrollIndicator(panel, offsetX, offsetY)
  ...
  local opacity = panel.sliderOpacity 
  fgColor = utils.withOpacity(fgColor, opacity or 0)
  local sliderW = 2
  local sliderH = (panel.h / panel:getActualSizeY()) * panel.h
  lg.setColor(fgColor)

  lg.rectangle('fill',
    panel.x + panel.w - sliderW - (panel.scrollDisplacement or 0),
    panel.y + (panel:getScrollY() * (panel.h - sliderH)),
    sliderW,
    sliderH
  )

  local sliderW = (panel.w / panel:getActualSizeX()) * panel.w
  local sliderH = 2
  lg.rectangle('fill',
    panel.x + (panel:getScrollX() * (panel.w - sliderW)),
    panel.y + panel.h - sliderH - (panel.scrollDisplacement or 0),
    sliderW,
    sliderH
  )
end
...
local function _drawDebug(panel)
  ...
  local cellW = (panel:getActualSizeX() / panel.cols)
...
function panel:draw()
  local scx, scy, cellWidth, cellHeight = lg.getScissor()
  ...
  lg.setScissor(scx, scy, cellWidth, cellHeight)
  lg.pop()
end
